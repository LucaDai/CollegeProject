/* FILE NAME: project2_Dai_mzd0108.cpp
* AUTHOR: Meiwen Dai
* A duel simulator calculate the best strategy for Aaron.
*/
#include <iostream>
#include <stdlib.h>
#include <assert.h>
...
//VARIABLES
const int A_accurate = 33;
const int B_accurate = 50;
const int C_accurate = 100;
const int TOTAL_RUNS = 10000;
bool aaronAlive = true; //Aaron alive
bool bobAlive = true; //Bob alive
bool charlieAlive = true; //Charlie alive
int aaronWins1 = 0; //track of strategy 1 for Aaron wins.
int bobWins = 0; //track for Bob wins.
int charlieWins = 0; //track for Charlie wins.
int aaronWins2 = 0; // Strategy 2 wins for Aaron.


int main() {
	//Initializes Random number generator's seed and calls test functions
	cout << "*** Welcome to Dai's Duel Simulator ***\n";
	srand(time(0));
	...
	* 100 << "%\n"
	...
	for (int i = 0; i < TOTAL_RUNS; i++) {
		...
		charlieAlive = true;
			while (at_least_two_alive(aaronAlive, bobAlive, charlieAlive)) {
				//Since A skips his first shoot, B shoot first.
				if (bobAlive) {
					Bob_shoots(aaronAlive, charlieAlive);
				}
